<template>
  <div>
    ...
    <v-table-selector ref="tableSelector" :on-ok="saveInputNode"></v-table-selector>
    <v-table-output-editor ref="outputEditor" :on-ok="saveOutputNode"></v-table-output-editor>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      contextMenu: {
        ...
        menu: [{
          ...
          // 空白处右键
          isShow: () => this.contextMenu.event.item == null,
          handler: () => this.addNode(this.contextMenu.event, 'output')
        }, {
          text: '编辑',
  ...
  methods: {
    render () {
      ...
      this.graph = new G6.Graph({
        ...
        modes: {
          ...
          zoom: ['zoom-canvas'/* 可以缩放画布 */, 'drag-node'/* 可以拖动节点 */,
            'drag-canvas'/* 可以拖动画布 */, 'click-select'/* 可以点击选中节点 */]
        },
        nodeStateStyles: {
          input: {
            fill: 'yellow',
      ...
      if (this.dataList == null || Object.entries(this.dataList).length === 0) {
        this.graph.getNodes().forEach(n => this.graph.setItemState(n, 'input', true))
      }
    },
    onMenuClicked (menu) {
      // 处理
    ...
    addNode (event, type = 'input') {
      ...
      this.setState(node, ['pending', type])
    ...
    beginEditNode (event) {
      ...
      console.log(this.editingNode.getStates())
      if (this.editingNode.getStates().indexOf('input') >= 0) {
        this.$refs.tableSelector.open(this.editingNode)
      } else {
        this.$refs.outputEditor.open(this.editingNode)
      }
  ...
  components: {
    'v-table-selector': () => import('@/components/table-selector'),
    'v-table-output-editor': () => import('@/components/table-output-editor'),
