...
export default {
  email(rule, value, callback) {
    return callback(emailReg.test(value) ? undefined : '请输入正确的邮箱')
  },
  url(rule, value, callback) {
    return callback(urlReg.test(value) ? undefined : '请输入正确的路径')
  },
  cellphone(rule, value, callback) {
    return callback(cellphoneReg.test(value) ? undefined : '请输入正确的手机号码')
  },
  password(rule, value, callback) {
    return callback(passwordReg.test(value) ? undefined : '密码过于简单，必须包含字母、数字、长度在6-20位')
  },
  idCard(rule, value, callback) {
    return callback(idCardReg.test(value) ? undefined : '请输入正确的身份证号码')
  }
};
