...
public class RedisConnectionBuilder {
    ...
    public static void of(RedisProxy proxy, Properties properties) {
        ...
        //最大空闲数
        poolConfig.setMaxIdle(YiayoStringUtils.parseInt(properties.getProperty("redis.maxIdle"), 200));
        poolConfig.setMinIdle(YiayoStringUtils.parseInt(properties.getProperty("redis.minIdle"), 10));
        poolConfig.setMaxTotal(YiayoStringUtils.parseInt(properties.getProperty("redis.maxTotal"), 1000));
        //最大建立连接等待时间
        poolConfig.setMaxWaitMillis(YiayoStringUtils.parseInt(properties.getProperty("redis.maxWait"), 1000));
        //是否在从池中取出连接前进行检验,如果检验失败,则从池中去除连接并尝试取出另一个
        poolConfig.setTestOnBorrow(Boolean.valueOf(properties.getProperty("redis.testOnBorrow")));

        Integer timeout = YiayoStringUtils.parseInt(properties.getProperty("redis.timeout"), 300000);
        Integer maxAttempts = YiayoStringUtils.parseInt(properties.getProperty("redis.maxAttempts"), 6);
