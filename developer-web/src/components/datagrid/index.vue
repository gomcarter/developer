  ...
  <table class="datagrid-container datagrid-frame" :style="{'width': width || 'auto'}">
    ...
    <tr class="datagrid-header-row" v-if="columns != null  && columns.length > 0">
      ...
      <th class="datagrid-header-check" v-if="checkable">
        <input type="checkbox" :disabled="(this.data || []).length === 0" v-model="checkAll" class="datagrid-checkbox">
      </th>
        ...
        if (this.dataUrl) {
          ...
          this.dataUrl(this.requestParams)
            .then((res) => {
              ...
              if(err.code === 'ECONNABORTED' && err.message === `timeout of ${err.request.timeout}ms exceeded`){
                loading.text = '请求失败'
                loading.customClass = "timeout"
                loading.spinner = 'el-icon-warning'
              }
            })
        }
...
<style lang="scss">
  .timeout{
    .el-loading-text, .el-icon-warning{
      color: #f56c6c;
      font-size: 20px;
    }
  }
